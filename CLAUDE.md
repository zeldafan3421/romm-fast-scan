...
- `.gitea/workflows/` mirrors both of the above for a self-hosted Gitea instance running its own **rootless-Podman** `act_runner` (label `ubuntu-latest`), publishing to the *same* `ghcr.io/zeldafan3421/romm-fast-scan` image rather than a separate registry. It can't be a byte-for-byte copy of the GitHub versions: no `docker/*`-actions or Docker daemon/socket exists under rootless Podman, so `build-container.yml` builds from `Containerfile` (not `Dockerfile`) via plain `podman build`/`login`/`push` CLI calls instead of `docker/setup-buildx-action`/`docker/login-action`/`docker/build-push-action`, with no `type=gha` cache (BuildKit-only, unsupported by Podman/buildah); Gitea's auto-injected `secrets.GITHUB_TOKEN` authenticates against the Gitea instance itself, not github.com/ghcr.io, so it needs a real GitHub PAT in a distinct `GHCR_TOKEN` secret for the registry login; and `gh`-CLI-based issue management in `check_upstream_versions.py` can't target a Gitea-hosted repo's (differently-shaped) issue API at all, so `compat-watch.yml` runs `scripts/check_upstream_versions_gitea.py` instead — a stdlib-`urllib` counterpart that still reads upstream releases from GitHub's public API but manages the tracking issue via Gitea's own REST API, using the same auto-injected token (valid here, since that call targets the Gitea instance itself). See the comments at the top of each `.gitea/workflows/*.yml` file for the full rationale. Keep the build/version-matrix steps in both `build-container.yml` files in sync when they change; only registry auth, container engine (podman vs docker/buildx), and the compat-watch script differ.

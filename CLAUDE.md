...
`romm-fast-scan` is a drop-in performance plugin for [RomM](https://github.com/rommapp/romm) (a ROM library manager), built on a small native-plugin system. RomM's `roms_handler.py` is patched **once** to call `plugin_manager.hash_file(...)` instead of hashing in pure Python; `plugin_manager.py` loads plain C-ABI `.so` plugins at runtime via `ctypes`. The bundled `fasthash` plugin does a file's whole read-and-hash in one native call, which `ctypes` runs with the GIL released, cutting the per-file Python overhead the stock path pays. The actual speedup is **modest and workload-dependent** — no single-file win (both paths bottom out in the same OpenSSL primitives; Python's own `hashlib`/`zlib` already release the GIL during the hash math), a measured ~1.5–1.6× warm-cache on many-small-file libraries at high `SCAN_WORKERS`, near-parity on few-large-file libraries, and *less* on a cold spinning disk (I/O-bound). Do not describe this as a big multiplier or claim stock RomM is fully GIL-serialized — that was an earlier, wrong framing; the reproducible numbers live in `tests/` (`sh tests/run.sh`). There's also an `archive-list` plugin (ZIP central-directory listing) that exists to prove the plugin system generalizes beyond hashing — it isn't wired into RomM's scan path yet, only proven at the loader/plugin level.
...
**Why `ctypes`, and what it costs.** The switch to `ctypes` bought that portability at a small, real, per-call price versus the old direct-C-API extension — worth writing down so it isn't later "discovered" and mistaken for a regression. A `ctypes` call goes through libffi and `plugin_manager.py`'s per-call Python wrapper (three `create_string_buffer`s, a `path.encode()`, three `.value.decode()`s); the old extension called through the native C-API and built its result tuple in C, avoiding most of that. Measured by `tests/call_overhead.py` on the dev VM: the new-vs-old differential is **~2 µs per `hash_file` call** (~2.1 µs wrapper + ~0.2 µs libffi floor; the ~5.7 µs of file-I/O syscalls per call is common to both designs and not part of the difference). That's **<1% of hashing time above ~130 KiB**, invisible on normal ROMs, and only a few percent on very small carts (sub-~10 KB Atari-2600/NES) — which is exactly why a small-ROM-heavy library reads as a *hair* slower under the `ctypes` build than the old extension, while both stay well ahead of stock RomM. It's the deliberate cost of dropping CPython-ABI coupling; if it ever mattered, the wrapper is the optimizable part (pre-allocated per-thread buffers, deferring the decode). Don't "fix" it speculatively — re-measure with `tests/call_overhead.py` first.
...
`tests/` is a stdlib-only synthetic suite, run with `sh tests/run.sh` (it builds the plugins and sets `FAST_SCAN_ALLOW_UNSIGNED_PLUGINS=1` for you, since local builds are unsigned):
- **`tests/correctness.py`** — verifies the native plugins agree with `hashlib`/`zlib`/`zipfile` across normal and edge-case inputs (`hash_file`, the accumulator's per-file *and* combined digests, `archive_list`). This is the foundation: a benchmark number is meaningless until correctness passes.
- **`tests/benchmark.py`** + **`tests/fixtures.py`** — native-vs-Python hashing across workload shapes and worker counts, printing a reproducible table.

**Rule that this suite exists to enforce: any performance number in the README/docs must be reproducible by `tests/benchmark.py`.** The repo previously carried an unsubstantiated "~3–5×" claim that propagated into several docs; it was wrong (measured reality is ~parity single-file, ~1.5–1.6× warm-cache on many-small-file libraries at high `SCAN_WORKERS`, less on cold disk). If you change hashing and want to quote a new figure, regenerate it here and quote it with its scope (warm-cache/CPU-bound is the upper bound; cold-disk is I/O-bound and lower) — never a bare multiplier. There's still no CI *gate* running this (`.github/workflows/build-container.yml` only builds/pushes images); it's a run-on-demand harness, matching the repo's manual-verification style.

When changing hashing logic, the fuller manual loop against a real RomM container:
1. Build an image, or use the volume-mount install against a real RomM instance, or (for isolated plugin work) just `sh scripts/build-plugins.sh` and load it directly (or `sh tests/run.sh`).
2. From the repo root: `sys.path.insert(0, "src"); import plugin_manager as pm; pm.load_plugins("plugins")`, then call `pm.hash_file(path)` / `pm.new_multi_file_accumulator()` / `pm.archive_list(path)` directly, comparing against `hashlib`/`zipfile` (this is what `tests/correctness.py` automates).
...
- `.gitea/workflows/` mirrors both of the above for a self-hosted Gitea instance running its own **rootless-Podman** `act_runner` (label `ubuntu-latest`), publishing to *that Gitea instance's own container registry* (derived at runtime from `github.server_url`/`github.repository`) rather than ghcr.io — a separate artifact from the GitHub-published image. It can't be a byte-for-byte copy of the GitHub versions: no `docker/*`-actions or Docker daemon/socket exists under rootless Podman, so `build-container.yml` builds from `Containerfile` (not `Dockerfile`) via plain `podman build`/`login`/`push` CLI calls instead of `docker/setup-buildx-action`/`docker/login-action`/`docker/build-push-action`, authenticated with Gitea's own auto-injected `secrets.GITHUB_TOKEN` (valid here since it's pushing to this same instance, unlike a foreign registry); every `podman build` also passes `--storage-opt overlay.ignore_chown_errors=true` to work around rootless Podman failing to extract multi-owner base-image layers (e.g. alpine's `/etc/shadow`) without a subuid/subgid range allocated to the runner's user — a real tradeoff (documented in the workflow's header comment) rather than a full fix, which is allocating that range (`usermod --add-subuids/--add-subgids`, `podman system migrate`) and dropping the flag; there's also no `type=gha` cache (BuildKit-only, unsupported by Podman/buildah). Separately, `gh`-CLI-based issue management in `check_upstream_versions.py` can't target a Gitea-hosted repo's (differently-shaped) issue API at all, so `compat-watch.yml` runs `scripts/check_upstream_versions_gitea.py` instead — a stdlib-`urllib` counterpart that still reads upstream releases from GitHub's public API but manages the tracking issue via Gitea's own REST API, using the same auto-injected token. See the comments at the top of each `.gitea/workflows/*.yml` file for the full rationale. Keep the build/version-matrix steps in both `build-container.yml` files in sync when they change; only registry target, container engine (podman vs docker/buildx), and the compat-watch script differ.

...
# Run RomM + the fast-scan plugin with plain `docker run` -- no compose file,
...
# install: volume mount" in README.md instead.
#
# Usage:
#   Edit the variables below, then:  sh scripts/run-docker.sh
...
i=0
until docker exec romm-db healthcheck.sh --connect --innodb_initialized >/dev/null 2>&1; do
    i=$((i + 1))
    if [ "$i" -ge 60 ]; then
        echo "ERROR: romm-db did not become healthy after 120s. Check: docker logs romm-db" >&2
        exit 1
    fi
    sleep 2
done
...
echo "Logs:      docker logs -f romm-app"
echo "Stop:      docker stop romm-app romm-db"
echo "Remove:    docker rm -f romm-app romm-db && docker network rm $NETWORK"
echo "  (the $DB_VOLUME volume is left in place -- add 'docker volume rm $DB_VOLUME' to also drop your database)"
